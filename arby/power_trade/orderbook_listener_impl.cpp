...
namespace power_trade
{
    ...
    orderbook_listener_impl::create(asio::any_io_executor exec, std::shared_ptr< connector > connector, json::string symbol)
    {
    ...
    orderbook_listener_impl::run(std::shared_ptr< orderbook_listener_impl > self)
    {
        ...
        cmd_response_conn_ = co_await connector_->watch_messages(
            "command_response",
            std::bind(&orderbook_listener_impl::_handle_command_response, weak_from_this(), std::placeholders::_1));
    ...
    orderbook_listener_impl::_handle_message(std::weak_ptr< orderbook_listener_impl > weak, message msg)
    {
        ...
        fmt::print("{}::{}({})\n",
                   ...
                   */
        if (auto self = weak.lock())
            asio::post(asio::bind_executor(self->get_executor(),
                                           std::bind(&orderbook_listener_impl::handle_message, self, std::move(msg))));
    }
    ...
    void
    orderbook_listener_impl::_handle_add(std::weak_ptr< orderbook_listener_impl > weak,
                                         std::shared_ptr< json::object const >    payload)
    {
    ...
    orderbook_listener_impl::on_add(json::object const &payload)
    {
        ...
        auto side  = wise_enum::from_string< side_type >(payload.at("side").as_string());
