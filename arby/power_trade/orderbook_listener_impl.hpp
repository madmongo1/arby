...
namespace power_trade
{
    ...
    , std::enable_shared_from_this< orderbook_listener_impl >
    {
        ...
        create(asio::any_io_executor exec, std::shared_ptr< connector > connector, json::string symbol);

        orderbook_listener_impl(asio::any_io_executor exec, std::shared_ptr< connector > connector, json::string symbol);
      ...
      private:
        ...
        {
            auto result =
                std::array< std::tuple< json::string, json::string >, 2 > { { { "symbol", symbol_ }, { "market_id", "0" } } };
            return result;
        }
