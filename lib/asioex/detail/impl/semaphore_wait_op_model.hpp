...
namespace detail
{
    ...
    semaphore_wait_op_model< Executor, Handler >::construct(async_semaphore_base *host, Executor e, Handler handler)
    {
        auto halloc = asio::get_associated_allocator(handler);
        auto alloc  = typename std::allocator_traits< decltype(halloc) >::template rebind_alloc< semaphore_wait_op_model >(halloc);
        auto traits = std::allocator_traits< decltype(alloc) >();
        auto pmem   = traits.allocate(alloc, 1);
        try
        {
            return new (pmem) semaphore_wait_op_model(host, std::move(e), std::move(handler));
        }
        catch (...)
        {
    ...
    template < class Executor, class Handler >
    auto
    semaphore_wait_op_model< Executor, Handler >::destroy(semaphore_wait_op_model *self) -> void
    {
    ...
    template < class Executor, class Handler >
    semaphore_wait_op_model< Executor, Handler >::semaphore_wait_op_model(async_semaphore_base *host, Executor e, Handler handler)
    ...
    , handler_(std::move(handler))
    {
        ...
        if (slot.is_connected())
            slot.assign(
                ...
                {
                    if (!!(type &
                           (asio::cancellation_type::terminal | asio::cancellation_type::partial | asio::cancellation_type::total)))
                    {
